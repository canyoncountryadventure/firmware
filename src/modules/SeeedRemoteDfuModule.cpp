...
#include "SeeedRemoteDfuModule.h"
...
void clearPending()
{
    ...
    confirmDestination = 0;
    confirmChannel = 0;
    confirmDueMs = 0;
    memset(previousBuild, 0, sizeof(previousBuild));
...
bool savePending(uint32_t destination, uint8_t channel)
{
    ...
    if (buildLength > BUILD_ID_MAX)
        buildLength = BUILD_ID_MAX;
    ...
    size_t written = 0;
    {
        ...
        if (!file) {
            LOG_WARN("Seeed DFU: open marker for write failed");
            return false;
        }
        if (!file.seek(0) || !file.truncate()) {
            LOG_WARN("Seeed DFU: truncate marker failed");
            file.close();
            return false;
        }
    ...
    if (written != sizeof(record)) {
        LOG_WARN("Seeed DFU: marker short write");
        return false;
    }
    ...
    bool extra = false;
    {
        ...
        if (!file) {
            LOG_WARN("Seeed DFU: marker reopen failed");
            return false;
        }
        verified = file.read(verify, sizeof(verify));
        extra = file.available();
        file.close();
    }

    if (extra || verified != sizeof(record) || memcmp(record, verify, sizeof(record)) != 0) {
        LOG_WARN("Seeed DFU: marker read-back verification failed");
        return false;
    }
    ...
    memset(previousBuild, 0, sizeof(previousBuild));
    memcpy(previousBuild, CURRENT_BUILD, buildLength);
    return true;
}
...
void loadPending()
{
...

    {
        ...
        if (!file)
            return;
        got = file.read(record, sizeof(record));
        extra = file.available();
        file.close();
    }
    ...
    if (extra ||
        got != sizeof(record) ||
        record[0] != 'D' || record[1] != 'F' || record[2] != 'U' || record[3] != '2' ||
        record[4] != 2 ||
        record[10] == 0 || record[10] > BUILD_ID_MAX ||
        record[31] != checksum(record, 31)) {
        LOG_WARN("Seeed DFU: invalid pending marker; clearing");
        clearPending();
        return;
    }
    ...
    confirmDestination = destination;
    confirmChannel = record[9];
    memcpy(previousBuild, &record[11], record[10]);
    previousBuild[record[10]] = '\0';
    confirmPending = true;
    confirmDueMs = millis() + CONFIRM_DELAY_MS;

    LOG_INFO("Seeed DFU: callback pending to=0x%08lX old=%s new=%s",
             static_cast<unsigned long>(confirmDestination), previousBuild, CURRENT_BUILD);
}
} // namespace

SeeedRemoteDfuModule::SeeedRemoteDfuModule()
    : SinglePortModule("seeed_remote_dfu", meshtastic_PortNum_TEXT_MESSAGE_APP),
      concurrency::OSThread("seeed_remote_dfu")
{
    isPromiscuous = true;
    loadPending();
...
bool SeeedRemoteDfuModule::wantPacket(const meshtastic_MeshPacket *p)
{
    return p != nullptr && p->decoded.portnum == meshtastic_PortNum_TEXT_MESSAGE_APP;
}

ProcessMessage SeeedRemoteDfuModule::handleReceived(const meshtastic_MeshPacket &mp)
{
    ...
    if (!savePending(mp.from, mp.channel)) {
        ...
        LOG_WARN("Seeed DFU: refusing reboot because callback marker was not verified");
        return ProcessMessage::CONTINUE;
    }

    sendTextReply(mp.from, mp.channel,
                  "BLE OTA DFU armed\nRebooting into XIAO DFU in 3 seconds\nMatched drone Scout can flash this target");

    rebootPending = true;
    rebootAtMs = millis() + REBOOT_DELAY_MS;
    setIntervalFromNow(10);

    LOG_WARN("Seeed DFU: armed by mesh DM from=0x%08lX channel=%u",
             static_cast<unsigned long>(mp.from), mp.channel);
    return ProcessMessage::CONTINUE;
}

bool SeeedRemoteDfuModule::sendTextReply(uint32_t destination, uint8_t channel, const char *text)
{
    if (destination == 0 || text == nullptr)
        return false;
...
int32_t SeeedRemoteDfuModule::runOnce()
{
    const uint32_t now = millis();

    if (rebootPending && reached(now, rebootAtMs)) {
        rebootPending = false;
        LOG_WARN("Seeed DFU: entering BLE OTA bootloader (GPREGRET=0xA8)");
        delay(100);
        nrf52FlashQuiesce();

        uint8_t enabled = 0;
        if (sd_softdevice_is_enabled(&enabled) == NRF_SUCCESS && enabled)
    ...
    if (confirmPending && confirmDestination != 0 && reached(now, confirmDueMs)) {
        const bool changed = previousBuild[0] != '\0' && strcmp(previousBuild, CURRENT_BUILD) != 0;
        char reply[220] = {};

        if (changed) {
            snprintf(reply, sizeof(reply),
                     "UPDATE SUCCESS\nSeeed XIAO booted new firmware after drone/BLE DFU\nOld:%s\nNew:%s",
                     previousBuild, CURRENT_BUILD);
        } else {
            snprintf(reply, sizeof(reply),
        ...
        if (sendTextReply(confirmDestination, confirmChannel, reply)) {
            LOG_INFO("Seeed DFU: post-update result queued");
            clearPending();
        } else {
            confirmDueMs = now + CONFIRM_RETRY_MS;
            LOG_WARN("Seeed DFU: callback enqueue failed; retry scheduled");
        }
    }
