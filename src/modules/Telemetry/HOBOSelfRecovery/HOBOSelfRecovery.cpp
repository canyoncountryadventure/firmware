...
static constexpr char FIRMWARE_LABEL[] = "SEEED WATER DISTANCE + HOBO V3";
...
ProcessMessage HOBOSelfRecoveryModule::handleReceived(const meshtastic_MeshPacket &mp)
{
    ...
    if (isCommand(payload, payloadSize, "HELP")) {
        ...
        sendTextReply(mp.from, mp.channel,
                      "HELP 2/3 WATER: WATER HELP | WATER STATUS | WATER CHECK | WATER INSTALL | WATER SENSOR | WATER SENSOR AUTO/SEN0590/SEN0311/SEN0313 (A01NYUB/A02YYUW) | WATER READ | WATER RAW | WATER VERIFY | WATER INTERVAL 1H");
        sendTextReply(mp.from, mp.channel,
                      "HELP 3/3: WATER CAL STATUS | WATER CAL STAGE 1.42FT | WATER CAL LOCK | WATER CAL UNLOCK | WATER CAL UNLOCK CONFIRM | WATER CAL RESET | WATER CAL RESET CONFIRM | WATER RESET WATER | WATER RESET WATER CONFIRM | WATER TELEMETRY NOW");
        return ProcessMessage::CONTINUE;
    }
    ...
    if (isCommand(payload, payloadSize, "VERSION")) {
        snprintf(reply, sizeof(reply), "%s\nHW:%s\nSensors:%s\nHOBO poll:30s | DFU:ON",
                 FIRMWARE_LABEL, platformName(), "HOBO + WATER DISTANCE");
        sendTextReply(mp.from, mp.channel, reply);
        return ProcessMessage::CONTINUE;
    }
