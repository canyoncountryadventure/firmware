# HOBO Peak Node — RAK4631 Self-Recovery Firmware

![Board](https://img.shields.io/badge/board-RAK4631%20%2F%20RAK19003-0A7F5A)
![Radio](https://img.shields.io/badge/radio-SX1262%20%2F%20Meshtastic-6A5ACD)
...
> **Branch:** `hobo-self-recovery-rak4631`
>
> **Target:** RAK4631 / WisBlock RAK19003-class field radio
>
...
This branch is the hardened RAK4631 field-node build for remote environmental stations. It uses the same proven universal HOBO protocol implementation as the Seeed build and adds a separate non-destructive recovery supervisor around it.
...
- The RAK build reuses the same universal three-model HOBO implementation rather than maintaining a divergent logger protocol fork.
...
The universal RAK implementation has already been validated with MX2201 automatic transmissions closely following a 20-second logger interval.
...
The disconnected scan mode is intentionally reduced to:
...
That matters for RAK solar stations expected to remain deployed for long periods.
...
1. Flash the **RAK4631 self-recovery** build.
...
```text
.github/workflows/build_hobo_self_recovery_rak4631.yml
```

Every push to this branch builds the RAK4631 target and uploads a firmware artifact. Treat a green workflow as the minimum software gate; bench testing with a real logger remains the hardware gate.
...
│   └── RAK translation shim for the shared implementation
│
├── MX2001Diagnostic.h
│   └── existing RAK Meshtastic hook name, now routed to universal reader
│
└── HOBOSelfRecovery/
...
| Branch | Purpose |
|---|---|
| `hobo-self-recovery-rak4631` | **This build — hardened RAK4631 field node** |
| `hobo-self-recovery-seeed` | Hardened Seeed XIAO field node |
